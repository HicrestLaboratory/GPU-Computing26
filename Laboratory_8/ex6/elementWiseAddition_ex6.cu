...
int main(int argc, char *argv[]) {
  ...
  // First loop: vary the array size, starting from 2^12, to 2^22
  //    1) set-up the variables
  //    2) set-up the problem: generate random float data
  //    3) computation on CPU: Write an vector addition function on the CPU side, and measure its CPU_time
  //    4) allocating GPU vectors
  //    Second loop: the range of block size: 32, 64, 128, 256, 512
  //        1) implement layout1: refer Ex5, store results
  //        2) implement layout3: refer Ex5, store results
  //    5) print the results

  for (/*vary the array size*/){   
    ...
    for(/* vary the block size */) {   // the range of block size: 32, 64, 128, 256, 512
