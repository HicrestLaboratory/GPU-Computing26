...
//   }
  ...
  TIMER_DEF;
  const char* lables[NPROBS] = {"CPU check", "Kernel 1", "Kernel 2", "Kernel 3", "Kernel 4", "CUB test", "CUB test 2", "CUB test 3"};
...
//     }
  ...
  GPU_b = execute_reduction_kernel(len, a, Kernel_CUB, &Bandwidths[5], &Times[5]);
  ...
  GPU_b = execute_reduction_kernel(len, a, Kernel_CUB2, &Bandwidths[6], &Times[6]);
  ...
  GPU_b = execute_reduction_kernel(len, a, Kernel_CUB3, &Bandwidths[7], &Times[7]);
